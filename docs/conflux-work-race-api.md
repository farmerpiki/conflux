...
owner-local trigger helpers when an owner already has a timer or shutdown event
source.

Use `race::timeout_after(duration)` only as a generic fallback timeout trigger.
...
Use `race::timeout_at(deadline)` for the same fallback behavior with an absolute
steady-clock deadline. All fallback helpers also accept an explicit label as
