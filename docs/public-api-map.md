...
New HTTP users can start with `import conflux;` in the default curated profile
or with the explicit `import conflux.http;` leaf façade.
...
| --- | --- | --- |
| Stable candidate | `import conflux;` / `<conflux.hxx>` | Selected aggregate surface, controlled by `CONFLUX_API_SURFACE`; default curated builds include the HTTP façade when built. |
