...
- `HttpServer::startup_report()` combines build info, capabilities, fallback policy, and redacted config for owner-controlled startup logging.

`Config::development()` keeps non-strict INI compatibility. `Config::public_server()` enables `strict_config`; strict checked loads report structured `ConfigIssue` values such as `config.unknown_key` with file, line, section, key, value, and hints where available.
...
`HttpServer::try_create()` rejects invalid config and fail-fast capability mismatches before constructing the server.
