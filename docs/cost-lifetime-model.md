...
| `http::file(...)` (`conflux.http.extended`) | File/static path or cached file entry owns backing source | Avoids body copies when served from file/mapped cache | May allocate metadata; may stat/open/cache lookup | Setup may do filesystem work | Yes for plain large file/mapped sends | TLS may disable kernel zero-copy paths | File path/cache entry must remain valid through send setup |
| `http::buffered_stream(...)` / `http::stream(...)` | Response owns a `std::string` built by `StreamSink` | Yes; chunks append into the response buffer | Yes as the buffer grows | Writer must not block ring thread | No; this is buffered, not incremental streaming | TLS still copies through TLS stack | Chunks only need to survive each `write()` call |
