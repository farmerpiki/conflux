...
| `http::Query<...>` / `RequiredQuery<...>` / `OptionalQuery<...>` | Usually no for view/scalar extraction | `req.query` | Server-owned dispatch/task lifetime for normalized app routes; otherwise caller-owned request lifetime | Yes for normalized app async handlers while the server-owned request remains live | No when borrowed | Only if copied/owned | String targets copy; numeric targets parse | String copies allocate; numeric success does not |
| `http::Header<...>` / `RequiredHeader<...>` / `OptionalHeader<...>` | Usually no for view/scalar extraction | `req.headers` | Server-owned dispatch/task lifetime for normalized app routes; otherwise caller-owned request lifetime | Yes for normalized app async handlers while the server-owned request remains live | No when borrowed | Only if copied/owned | String targets copy; numeric targets parse | String copies allocate; numeric success does not |
| `http::Cookie<...>` / `RequiredCookie<...>` / `OptionalCookie<...>` | Usually no for view/scalar extraction | `req.cookies` | Server-owned dispatch/task lifetime for normalized app routes; otherwise caller-owned request lifetime | Yes for normalized app async handlers while the server-owned request remains live | No when borrowed | Only if copied/owned | String targets copy; numeric targets parse | String copies allocate; numeric success does not |
| `http::Form<...>` / `RequiredForm<...>` / `OptionalForm<...>` | Usually no for view/scalar extraction | `req.form` | Server-owned dispatch/task lifetime for normalized app routes; otherwise caller-owned request lifetime | Yes for normalized app async handlers while the server-owned request remains live | No when borrowed | Only if copied/owned | String targets copy; numeric targets parse | String copies allocate; numeric success does not |
...
| `http::OwnedBodyBytes` | Yes | Nothing after extraction | Object lifetime | Yes | Yes, subject to normal C++ synchronization | Yes | Yes, from request body | Yes |
...
| `http::JsonDocument` | Yes | Parsed request body document | Object lifetime | Yes | Yes, subject to normal document/thread-safety rules | Yes | Yes, from parsing request body | Yes |
...
| `http::cookie(...)` / response-builder `.cookie(...)` / `Response::set_cookie(...)` | Response owns formatted `Set-Cookie` header strings | Builder stores name/value/attributes, then the response builder or `set_cookie` formats one header string | Yes for cookie/header strings | No | Not relevant | Not relevant | Caller data only needs to survive the builder call |
| `http::no_content()` | No body | No | Minimal response metadata only | No | Not relevant | Not relevant | No body data |
| `http::blocking_file_response(...)` / `http::blocking_file_response(root, relative_path, ...)` (`conflux.http.extended`) | Response owns a `std::string` body read from a regular file | Yes; reads the file into the response body up to the configured limit | Yes for the body buffer | Yes; performs blocking filesystem I/O on the caller | No; use static/file streaming paths for zero-copy or async sends | TLS still copies through TLS stack | Use the root-relative overload for request-derived paths; it rejects traversal and symlink escapes before copying file contents |
...
| `parse_copy` / `parse_owned` / owning document | Document owns copied or moved input | Strings refer to document-owned storage or provider storage | Provider dependent | Copies lvalue input; moves rvalue input | Yes for owned buffer/metadata | May allocate diagnostic text | Strict validation unless options/provider say otherwise | Controlled by parse options; default should reject |
...
|---|---|---|---|---|---|---|
| `file_map::MappedFileLease` | Shared lease owns the mapped region handle | Lease lifetime | No file-body copy | Yes for lease/control metadata | Mapping setup may block on filesystem metadata/page setup | Page faults may happen later when bytes are touched |
| Static mapped response | Response owns a lease to the mapped file region | Response send completion or cancellation | No file-body copy on the plain path | Metadata/cache entries may allocate | `stat`, `open`, cache lookup, and range handling may block unless offloaded/cached | TLS may bypass kernel zero-copy and copy through TLS buffers |
| Static streamed file response | Response owns a streamed-file handle | Response send completion or cancellation | No whole-body copy on the plain path | Metadata/control state allocates | `open`/metadata setup can block unless offloaded/cached | TLS uses fixed-buffer reads and TLS writes rather than splice/sendfile |
...
| `http::offload` / worker pool path (`conflux.http.extended`) | Enqueues work from caller; work runs on worker | Caller should not block while queued work runs | No for the offloaded work | Yes | Yes for blocking/heavy work that has been explicitly moved |
...
| `conflux::http::HttpServer::drain` | Caller requests drain; rings perform connection work | No long wait in the API itself; `run()` returns when rings finish | Wakes ring threads; deadline close work runs on rings | Yes | Yes from a controller thread, not from a handler |
