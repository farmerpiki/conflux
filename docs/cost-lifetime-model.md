...
| `http::BodyBytes` | No for view/span form | Request body buffer | End of synchronous handler call, unless copied | No when borrowed | No when borrowed | Only if copied/owned | No for view form | No for view form |
...
| `http::file(...)` | File/static path or cached file entry owns backing source | Avoids body copies when served from file/mapped cache | May allocate metadata; may stat/open/cache lookup | Setup may do filesystem work | Yes for plain large file/mapped sends | TLS may disable kernel zero-copy paths | File path/cache entry must remain valid through send setup |
| `http::stream(...)` | Stream producer owns or controls chunks | Producer dependent | Producer dependent | Producer must not block ring thread | Usually no; chunked path dependent | TLS sends through TLS path | Produced chunks must stay valid until accepted by the stream send path |
...
app.get("/bad", [] {
    ...
    return http::stream(std::string_view{body});
});
```
