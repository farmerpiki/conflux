# `conflux.work` API Redesign (Proposal V7)

Status: proposal from v6 plus final polish review synthesis

This document replaces
[`conflux-work-api-redesign-proposal-v6.md`](./conflux-work-api-redesign-proposal-v6.md)
...
V7 keeps the root model intentionally narrow, but closes the remaining
polish-level gaps from V6:
...
- V7 does not lock triviality propagation or trivial ABI for `Outcome<T>`, even
...
V7 keeps the explicit extension point for control-block allocation and future
...
rule, but V7 does not require shared ownership, wrapper indirection, or a
...
generic "wait until all hooks for this subsystem are quiescent" API.
...
That behavior remains a hard contract in V7. The root layer does not
...
The V2/V3 `background(...)` conversion spelling is retired. V7 uses only
...
- V7 intentionally does not add a root-level `heavy_work_value` trait or
...
V7 intentionally does not define a destructor-joining scope guard in the root
...
for standard stop-token-aware code; V7 does not add a separate
...
Affinity is semantic, but V7 still keeps the root syntax narrower than the old
replacement draft.
...
V7 makes one narrowing explicit: cooperative code observes only the stop signal.
...
to satisfy the standard sender concepts in V7.
...
Later layers may provide explicit adapters to `std::execution`, but V7 does not
...
`control.stop_token()`. V7 still does not promise sender queries or
...
- subsystems are responsible for keeping enough live-control-block bookkeeping
  to fulfill the shutdown liveness invariant above; V7 does not standardize a
...
V6 was already implementable, but it still left a few polish-level
clarifications open for implementers reviewing edge cases.

V7 keeps the same narrow scope and makes those last details explicit:
