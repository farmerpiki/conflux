# `conflux.work` API Redesign (Proposal V4)

Status: proposal from v3 plus follow-up review synthesis

This document replaces
[`conflux-work-api-redesign-proposal-v3.md`](./conflux-work-api-redesign-proposal-v3.md)
...
V4 keeps the root model intentionally narrow, but closes the remaining
load-bearing gaps from V3:
...
- `Outcome<T>::success()`, `failure()`, and `cancelled()` have the obvious arm
  preconditions. Violating those preconditions is a contract violation. Use
  `kind()` / `is_*()` / `visit(...)` for total inspection.
...
V4 keeps the explicit extension point for control-block allocation and future
...
- `make_posted_source(...)` and `make_operation_source(...)` bind the required
  owner/driver identity at construction time and store that identity in the
  control block
...
That behavior remains a hard contract in V4. The root layer does not
...
The V2/V3 `background(...)` conversion spelling is retired. V4 uses only
...
- the sink must be invocable as `std::invocable<Sink&, Failure const&>`
- `Failure` is delivered to the sink
- `Cancelled` is delivered only if `std::invocable<Sink&, Cancelled const&>`
  holds for the sink type
...
`scoped_abandon<...>` is move-only and `noexcept`-destructible.

If still armed at destruction, it performs:

- `abandon_to(std::move(result), std::move(sink))`

The guard's `release()` member transfers the wrapped root object or join handle
back to the caller and disarms the destructor path.

This is the standard RAII tool for exception-safe root-object cleanup on stack
unwinding.

V4 intentionally does not define a destructor-joining scope guard in the root
...
concept progress_capability =
    requires(Cap const& cap) {
        { capability_id(cap) } -> std::totally_ordered;
    };
```

The control block stores the comparable capability identity chosen at admission
or producer-source construction time. `join(...)` and `can_join(...)` compare
the stored identity against `capability_id(...)` for the supplied owner/driver.
...
It does not check transient same-thread reentrancy. A `true` result from
`can_join(...)` does not guarantee that `join(...)` cannot still throw
`JoinContextError` for reentrancy.
...
equivalent mechanism. Cross-owner and cross-driver deadlocks are not detected at
the root layer.

## Affinity

Affinity is semantic, but V4 still keeps the root syntax narrower than the old
replacement draft.
...
V4 makes one narrowing explicit: cooperative code observes only the stop signal.
...
to satisfy the standard sender concepts in V4.
...
Later layers may provide explicit adapters to `std::execution`, but V4 does not
...
V3 got the category split and the linear lifetime direction right, but it still
left a few core contracts underspecified.

V4 keeps the same narrow scope and makes the contract implementable:
...
- cancel-hook, PMR lifetime, and join-context races are specified instead of
  implied
