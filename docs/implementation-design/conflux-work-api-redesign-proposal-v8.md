# `conflux.work` API Redesign (Proposal V8)

Status: proposal from v7 plus final precision review synthesis

This document replaces
[`conflux-work-api-redesign-proposal-v7.md`](./conflux-work-api-redesign-proposal-v7.md)
...
V8 keeps the root model intentionally narrow, but closes the remaining
polish-level gaps from V7:
...
- if copying a copyable `Outcome<T>` throws because copying `T` throws, that
  exception propagates, the destination is not constructed, and the source
  remains unchanged. No failed copy produces a valueless `Outcome<T>`.
- implementations must provide that copy-construction guarantee with internal
  staging equivalent to placement construction plus rollback on exception, not
  with naive member-wise mutation of an already-observable destination object
...
- V8 does not lock triviality propagation or trivial ABI for `Outcome<T>`, even
...
V8 keeps the explicit extension point for control-block allocation and future
...
rule, but V8 does not require shared ownership, wrapper indirection, or a
...
That behavior remains a hard contract in V8. The root layer does not
...
The V2/V3 `background(...)` conversion spelling is retired. V8 uses only
...
- V8 intentionally does not add a root-level `heavy_work_value` trait or
...
V8 intentionally does not define a destructor-joining scope guard in the root
...
for standard stop-token-aware code; V8 does not add a separate
...
copy/move behavior required of standard exception objects. Concrete headers are
expected to spell those special members explicitly enough to keep that
non-throwing guarantee visible rather than depending on accidental implicit
generation.
...
struct capability_id_from_address {
    static void type_tag() noexcept {}

    friend auto tag_invoke(capability_id_t, Derived const& self) noexcept
        -> std::pair<void (*)() noexcept, void const*> {
        return {&type_tag, std::addressof(self)};
    }
};
...
Affinity is semantic, but V8 still keeps the root syntax narrower than the old
replacement draft.
...
V8 makes one narrowing explicit: cooperative code observes only the stop signal.
...
to satisfy the standard sender concepts in V8.
...
Later layers may provide explicit adapters to `std::execution`, but V8 does not
...
`control.stop_token()`. V8 still does not promise sender queries or
...
- subsystems are responsible for keeping enough live-control-block bookkeeping
  to fulfill the shutdown liveness invariant above; V8 does not standardize a
...
V7 was already implementable, but it still left a few polish-level
clarifications open for implementers reviewing edge cases.

V8 keeps the same narrow scope and makes those last details explicit:
...
- outcome-copy semantics now include the required strong exception-safety model
