# `conflux.work` API Redesign (Proposal V9)

Status: proposal from v8 plus final nitpick pass

This document replaces
[`conflux-work-api-redesign-proposal-v8.md`](./conflux-work-api-redesign-proposal-v8.md)
...
V9 keeps the root model intentionally narrow, but closes the remaining
nitpick-level gaps from V8:
...
- V9 does not lock triviality propagation or trivial ABI for `Outcome<T>`, even
...
V9 keeps the explicit extension point for control-block allocation and future
...
rule, but V9 does not require shared ownership, wrapper indirection, or a
...
V9 does not require the standard library implementation's internal
...
That behavior remains a hard contract in V9. The root layer does not
...
The V2/V3 `background(...)` conversion spelling is retired. V9 uses only
...
- V9 intentionally does not add a root-level `heavy_work_value` trait or
...
V9 intentionally does not define a destructor-joining scope guard in the root
...
for standard stop-token-aware code; V9 does not add a separate
...
Affinity is semantic, but V9 still keeps the root syntax narrower than the old
replacement draft.
...
V9 makes one narrowing explicit: cooperative code observes only the stop signal.
...
to satisfy the standard sender concepts in V9.
...
Later layers may provide explicit adapters to `std::execution`, but V9 does not
...
`control.stop_token()`. V9 still does not promise sender queries or
...
- subsystems are responsible for keeping enough live-control-block bookkeeping
  to fulfill the shutdown liveness invariant above; V9 does not standardize a
...
V8 was already implementable, but it still left a few precision-level contract
edges open for implementers reviewing corner cases.

V9 keeps the same narrow scope and makes those last details explicit:
