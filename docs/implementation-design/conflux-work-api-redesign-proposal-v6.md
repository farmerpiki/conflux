# `conflux.work` API Redesign (Proposal V6)

Status: proposal from v5 plus follow-up review synthesis

This document replaces
[`conflux-work-api-redesign-proposal-v5.md`](./conflux-work-api-redesign-proposal-v5.md)
...
V6 keeps the root model intentionally narrow, but closes the remaining
polish-level gaps from V5:
...
- V6 does not lock triviality propagation or trivial ABI for `Outcome<T>`, even
...
V6 keeps the explicit extension point for control-block allocation and future
...
rule, but V6 does not require shared ownership, wrapper indirection, or a
...
That behavior remains a hard contract in V6. The root layer does not
...
The V2/V3 `background(...)` conversion spelling is retired. V6 uses only
...
V6 intentionally does not define a destructor-joining scope guard in the root
...
for standard stop-token-aware code; V6 does not add a separate
...
template<class T>
auto value(Task<T>&& task) noexcept(false)
...
template<class T>
auto value(TaskJoinHandle<T>&& h) noexcept(false)
    -> std::conditional_t<std::is_void_v<T>, void, T>;
...
copy/move behavior required of standard exception objects.
...
address identity is the intended capability identity.
...
Affinity is semantic, but V6 still keeps the root syntax narrower than the old
replacement draft.
...
V6 makes one narrowing explicit: cooperative code observes only the stop signal.
...
to satisfy the standard sender concepts in V6.
...
Later layers may provide explicit adapters to `std::execution`, but V6 does not
...
- subsystems are responsible for keeping enough live-control-block bookkeeping
  to fulfill the shutdown liveness invariant above; V6 does not standardize a
...
V5 got the category split and the linear lifetime direction right, but it still
left a few polish-level clarifications open.

V6 keeps the same narrow scope and makes those last details explicit:
