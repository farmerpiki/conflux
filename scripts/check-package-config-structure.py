...
def check_build_cost_release_evidence() -> None:
    ...
    required_reporter = {
        ...
        '"host": {': "build-cost reporter must record host metadata",
        '"kernel": platform.release()': "build-cost reporter must record kernel metadata",
        '"cpu_model": cpu_model()': "build-cost reporter must record CPU metadata",
        '"ram_bytes": ram_bytes()': "build-cost reporter must record RAM metadata",
        '"tools": {': "build-cost reporter must record build tool metadata",
        '"ninja": tool_version(["ninja", "--version"])': "build-cost reporter must record Ninja version",
        '"environment": {': "build-cost reporter must record cache-wrapper metadata",
        '"ccache": tool_version(["ccache", "--version"])': "build-cost reporter must record ccache availability",
        '"sccache": tool_version(["sccache", "--version"])': "build-cost reporter must record sccache availability",
...
def check_build_docs_guard_contracts() -> None:
    checks = {
        ...
        "tests/BuildAndDocsChecks.cmake": {
            ...
            "conflux_add_python_check(docs/public-api-naming": "missing public API naming CTest guard",
