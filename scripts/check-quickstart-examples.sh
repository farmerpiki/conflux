...
if grep -R -n -E 'conflux\.types|HttpRequest|HttpResponse|WorkPool|TaskSource|std::this_thread::sleep_for|json_response\(|http::Json\{|http::ok\(|http::codec::json|Config::(low_latency|benchmark|unsafe_max_speed)' "$root"; then
