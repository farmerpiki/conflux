...
usage() {
    printf 'usage: %s --variant VARIANT --evidence-dir DIR [--build-dir DIR] [--log-prefix PATH]\n' "$0" >&2
    printf '  VARIANT: gcc16 | clang-header | gcc15\n' >&2
}
...
if [[ -z "$variant" || -z "$evidence_dir" ]]; then
    usage; exit 2
fi

case "$variant" in
    gcc16)
        proof_subdir="json-proof-gcc16"
        compiler_bin="g++-16"
    ...
    clang-header)
        ...
        compiler_bin="/usr/lib/llvm/21/bin/clang++"
...
require_file() {
    local path="$1"
    if [[ ! -f "$path" ]]; then
        printf 'record-json-proof-evidence-variant: missing required file: %s\n' "$path" >&2
        exit 1
    fi
}
...
require_dir() {
    local path="$1"
    if [[ ! -d "$path" ]]; then
        printf 'record-json-proof-evidence-variant: missing required directory: %s\n' "$path" >&2
        exit 1
    fi
}
...
proof_dir="$evidence_dir/$proof_subdir"
...
if [[ -n "$scan_output" ]]; then
    ...
    printf 'record-json-proof-evidence-variant: warning/error scan matched; leaving build tree at %s\n' "$build_dir" >&2
    exit 1
fi
...

{
    ...
    case "$variant" in
        gcc16)
            printf 'cmake -Wno-dev -S . -B %q -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER=g++-16 -DCONFLUX_USE_IMPORT_STD=OFF -DCONFLUX_FEATURE_SET=dev-json -DCONFLUX_BUILD_TESTS=ON -DCONFLUX_BUILD_EXAMPLES=OFF -DCONFLUX_BUILD_BENCHMARKS=OFF -DCONFLUX_FETCH_TEST_DEPS=ON -DCONFLUX_TEST_CATCH2_PROVIDER=FETCH -DCONFLUX_ENABLE_JSON_TESTSUITE=ON > %q 2>&1\n' \
                "$build_dir" "$configure_log" ;;
        clang-header)
            printf 'cmake -Wno-dev -S . -B %q -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER=/usr/lib/llvm/21/bin/clang++ -DCMAKE_CXX_FLAGS=%q -DCMAKE_EXE_LINKER_FLAGS=%q -DCMAKE_SHARED_LINKER_FLAGS=%q -DCONFLUX_INTERFACE_MODE=HEADER_INTERFACE -DCONFLUX_FEATURE_SET=dev-json -DCONFLUX_BUILD_TESTS=ON -DCONFLUX_BUILD_EXAMPLES=OFF -DCONFLUX_BUILD_BENCHMARKS=OFF -DCONFLUX_FETCH_TEST_DEPS=ON -DCONFLUX_TEST_CATCH2_PROVIDER=FETCH -DCONFLUX_ENABLE_JSON_TESTSUITE=ON > %q 2>&1\n' \
                "$build_dir" \
                ...
                "$configure_log" ;;
        gcc15)
            printf 'cmake -Wno-dev -S . -B %q -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER=g++ -DCONFLUX_USE_IMPORT_STD=OFF -DCONFLUX_FEATURE_SET=dev-json -DCONFLUX_BUILD_TESTS=ON -DCONFLUX_BUILD_EXAMPLES=OFF -DCONFLUX_BUILD_BENCHMARKS=OFF -DCONFLUX_FETCH_TEST_DEPS=ON -DCONFLUX_TEST_CATCH2_PROVIDER=FETCH -DCONFLUX_ENABLE_JSON_TESTSUITE=ON > %q 2>&1\n' \
                "$build_dir" "$configure_log" ;;
    esac
    ...
    printf 'scripts/record-json-proof-evidence-variant.sh --variant %s --evidence-dir %q --build-dir %q --log-prefix %q\n' "$variant" "$evidence_dir" "$build_dir" "$log_prefix"
...
printf 'record-json-proof-evidence-variant: ok (%s, %s)\n' "$variant" "$commit"
