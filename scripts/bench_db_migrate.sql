...
VALUES
    ...
    ('db_protocol_synthetic', '*', '*', 7.5, 5, 15.0, 'DB-independent PostgreSQL protocol decode rows'),
    ('file_copy_coro', '*', '*', 15.0, 2, 25.0, 'filesystem/runtime benchmark'),
    ('http_app_path', '*', '*', 7.5, 5, 15.0, 'pure user-space HTTP app-path floor'),
    ('http_adversarial', '*', '*', 10.0, 5, 20.0, 'pure user-space HTTP adversarial parser/security path'),
    ...
    ('kernel_state_synthetic', '*', '*', 7.5, 5, 15.0, 'no-kernel fd/generation/deferred-close/CQE state floor'),
    ('router', '*', '*', 5.0, 5, 12.0, 'hot-path route lookup'),
    ('send_zc', '*', '*', 20.0, 1, 100.0, 'transport threshold sweep has expensive rows'),
    ('slow_consumer_backpressure', '*', '*', 20.0, 1, 100.0, 'live slow-client/backpressure policy rows'),
    ('static_strategy_matrix', '*', '*', 20.0, 1, 100.0, 'live static-file strategy matrix'),
    ('storage_read', '*', '*', 20.0, 1, 100.0, 'NVMe/O_DIRECT storage-read gate'),
    ('synthetic_cqe_coro', '*', '*', 10.0, 5, 20.0, 'no-kernel coroutine/CQE plumbing floor'),
    ...
    ('tls_mem_bio', '*', '*', 10.0, 5, 20.0, 'no-socket steady-state TLS encode/decode floor'),
    ('tls_tcp_increment_coro', '*', '*', 20.0, 5, 30.0, 'TLS local TCP coroutine path'),
    ('uring_completion', '*', '*', 7.5, 5, 15.0, 'no-kernel CompletionTable dispatch floor'),
    ('uring_timeout', '*', '*', 15.0, 5, 25.0, 'live timeout-remove miss with NOP baseline'),
