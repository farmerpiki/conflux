...
if grep -n -E 'conflux\.types|json_response\(|Response::json\(R"\(\{|http::Json\{|http::ok\(|http::codec::json|Config::(low_latency|benchmark|unsafe_max_speed)|std::this_thread::sleep_for|http::defer\(' "$public_root"/*.cxx; then
...
for file in "$public_root"/hello.cxx "$public_root"/forms.cxx "$public_root"/sse.cxx "$public_root"/static.cxx; do
