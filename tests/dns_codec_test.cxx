...
TEST_CASE(
	...
	"[dns][codec]") {
	vector<u8> wire = {
		0x00, 0x02, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 2,    'v',  '6',  1,
		'x',  0, // v6.x — labels: "v6" then "x"
...
TEST_CASE(
	...
	"[dns][codec][safety]") {
	vector<u8> wire = {
		0x00, 0x01, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 1,    'a',  0,    0x00,
		0x01, 0x00, 0x01, 0xC0, 0x0C, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x3C, 0xFF, 0xFF, // RDLENGTH 65535 —
																								  // overruns wire
	};
	CHECK_THROWS_AS(decode_message(wire), DnsError);
}
