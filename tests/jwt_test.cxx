...
TEST_CASE(
	...
	auto token = conflux::http::jwt_sign_unchecked(payload, secret);
...
TEST_CASE(
	...
	auto token = conflux::http::jwt_sign_unchecked(header, payload, secret);
...
TEST_CASE(
	...
	auto token = conflux::http::jwt_sign_unchecked(payload, "correct-secret-32bytes");
...
TEST_CASE(
	...
	auto missing_claims = conflux::http::jwt_decode(conflux::http::jwt_sign_unchecked(R"({"sub":"u"})", secret), opts);
	REQUIRE_FALSE(missing_claims.has_value());
	CHECK(missing_claims.error() == "missing exp claim");

	auto valid = conflux::http::jwt_decode(
		conflux::http::jwt_sign_unchecked(
	...
	auto too_long = conflux::http::jwt_decode(
		conflux::http::jwt_sign_unchecked(
			std::format(R"({{"sub":"u","jti":"long","iat":{},"exp":{}}})", now, now + 600),
			secret),
		opts);
...
TEST_CASE(
	...
	auto exp = conflux::http::jwt_decode(conflux::http::jwt_sign_unchecked(R"({"sub":"u","exp":-1})", secret), opts);
	REQUIRE_FALSE(exp.has_value());
	CHECK(exp.error() == "invalid exp claim");

	auto nbf = conflux::http::jwt_decode(conflux::http::jwt_sign_unchecked(R"({"sub":"u","nbf":-1})", secret), opts);
	REQUIRE_FALSE(nbf.has_value());
	CHECK(nbf.error() == "invalid nbf claim");

	auto iat = conflux::http::jwt_decode(conflux::http::jwt_sign_unchecked(R"({"sub":"u","iat":-1})", secret), opts);
...
TEST_CASE(
	...
	SECTION("alg none with valid HMAC is still rejected") {
		auto token = conflux::http::jwt_sign_unchecked(R"({"alg":"none","typ":"JWT"})", R"({"sub":"u"})", secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
		CHECK(result.error() == "unsupported algorithm (only HS256 supported)");
	}
	SECTION("RS256 header with valid HMAC is still rejected") {
		auto token = conflux::http::jwt_sign_unchecked(R"({"alg":"RS256","typ":"JWT"})", R"({"sub":"u"})", secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
		CHECK(result.error() == "unsupported algorithm (only HS256 supported)");
	}
	SECTION("duplicate alg header is rejected before choosing either value") {
		auto token =
			conflux::http::jwt_sign_unchecked(R"({"alg":"HS256","alg":"none","typ":"JWT"})", R"({"sub":"u"})", secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
...
TEST_CASE(
	...
	SECTION("duplicate subject") {
		auto token = conflux::http::jwt_sign_unchecked(R"({"sub":"victim","sub":"attacker"})", secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("duplicate expiration") {
		auto const now = jwt_test_now();
		auto token =
			conflux::http::jwt_sign_unchecked(std::format(R"({{"sub":"u","exp":{},"exp":1}})", now + 3600), secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("nested object keys with the same spelling are not treated as duplicate top-level claims") {
		auto token = conflux::http::jwt_sign_unchecked(R"({"sub":"u","nested":{"sub":"inner"}})", secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE(result.has_value());
		CHECK(result->sub == "u");
	}
...
TEST_CASE(
	...
	auto decoded = conflux::http::jwt_decode(
		conflux::http::jwt_sign_unchecked(R"({"sub":"user\u0031","iss":"issuer\u002dA"})", secret),
		opts);
	...
	auto audience = conflux::http::jwt_decode(
		conflux::http::jwt_sign_unchecked(R"({"sub":"u","aud":["api\u002dv1"]})", secret),
		opts);
...
TEST_CASE(
	...
	auto result = conflux::http::jwt_decode(
		conflux::http::jwt_sign_unchecked(R"({"sub":"victim","\u0073ub":"attacker"})", secret),
		opts);
	REQUIRE_FALSE(result.has_value());
	CHECK(result.error() == "duplicate sub claim");
}
...
TEST_CASE(
	...
	SECTION("missing exp") {
		auto result = conflux::http::jwt_decode(
			conflux::http::jwt_sign_unchecked(std::format(R"({{"sub":"u","iat":{}}})", jwt_test_now()), secret),
			opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("missing iat") {
		auto result = conflux::http::jwt_decode(
			conflux::http::jwt_sign_unchecked(std::format(R"({{"sub":"u","exp":{}}})", jwt_test_now() + 60), secret),
			opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("out-of-range exp") {
		auto result = conflux::http::jwt_decode(
			conflux::http::jwt_sign_unchecked(R"({"sub":"u","iat":1,"exp":999999999999999999999999999999})", secret),
			opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("out-of-range iat") {
		auto result = conflux::http::jwt_decode(
			conflux::http::jwt_sign_unchecked(
				R"({"sub":"u","iat":999999999999999999999999999999,"exp":9999999999})",
				secret),
			opts);
		REQUIRE_FALSE(result.has_value());
	...
	SECTION("future lifetime exceeds cap") {
		auto const now = jwt_test_now();
		auto token = conflux::http::jwt_sign_unchecked(
			std::format(R"({{"sub":"u","iat":{},"exp":{}}})", now, now + 86400),
			secret);
		auto result = conflux::http::jwt_decode(token, opts);
		REQUIRE_FALSE(result.has_value());
