...
static_assert(
static_cast<int>(root::AbandonStatus::installed)>=0||static_cast<int>(root::AbandonStatus::already_abandoned)>=0);
}// namespace snapshot_root
// ---------------------------------------------------------------------------
...
auto chain=carrier::from_task(move(task));
auto result=
move(chain).transform_outcome([](root::Outcome<int>out){return root::Outcome<int>{move(out)};});
static_assert(same_as<decltype(result),carrier::Chain<int>>);
(void)result;
}
