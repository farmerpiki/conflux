...
TEST_CASE(
	...
	{ carrier::DroppableSlot<int> const slot{move(jh)}; } // drain installed; handle will be consumed when src commits
...
TEST_CASE(
	...
	auto jh = root::into_join_handle(move(task));
	{ carrier::DroppableSlot<int> const slot{move(jh)}; } // ready path: join inline in dtor
}
TEST_CASE(
...
TEST_CASE(
	...
	auto [task, src] = root::make_task_source<int>();
	auto jh = root::into_join_handle(move(task));
	{ carrier::DroppableSlot<int> const slot{move(jh)}; } // drain hook installed
...
TEST_CASE(
	...
	auto jh = root::into_join_handle(move(task));
	auto coro = coro_droppable_slot(carrier::DroppableSlot<int>{move(jh)});
	auto chain = coro.get();
	auto out = move(chain).release_outcome();
...
TEST_CASE(
	"phase8f: co_await DroppableSlot async path — commit from thread resumes coroutine",
	"[phase8f]") {
	auto [task, src] = root::make_task_source<int>();
	auto jh = root::into_join_handle(move(task));
	auto coro = coro_droppable_slot(carrier::DroppableSlot<int>{move(jh)});
...
TEST_CASE(
	...
	carrier::DroppableSlot<int> slot{move(jh)};
	slot.on_drop([&](root::Outcome<int> const &) noexcept { fired = true; });
	auto coro = coro_droppable_slot(move(slot));
	auto chain = coro.get();
	(void)chain;
	CHECK_FALSE(fired);
}
