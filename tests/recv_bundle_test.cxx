...
u32 head_flags(BufferRing&ring)noexcept{
...
ring{uring.ref(),BufferRingOptions{.count=count,.buf_size=buf_size,.group_id=gid,.huge_pages=false,.mode=BufferRingMode::classic_one_cqe_per_buffer}}{}
};
template<typename F>
struct ScopeExit{
F fn;
...
TEST_CASE("recv_bundle.classic: head advances by 1 per CQE","[recv_bundle]"){
Rig rig{8,64};
for(u32 i=0;i<8;++i){
...
TEST_CASE("recv_bundle.bundle: single-buffer CQE","[recv_bundle]"){
Rig rig{8,64};
...
TEST_CASE("recv_bundle.bundle: exact 3 full buffers","[recv_bundle]"){
Rig rig{8,64};
...
TEST_CASE("recv_bundle.bundle: partial last buffer","[recv_bundle]"){
Rig rig{8,64};
...
SZ total=0;
for(auto const&s:slices){
if(idx<2)CHECK(s.bytes.size()==64u);
else CHECK(s.bytes.size()==37u);
total+=s.bytes.size();
++idx;
}
...
TEST_CASE("recv_bundle.bundle: ring wraparound","[recv_bundle]"){
Rig rig{4,64};
...
TEST_CASE("recv_bundle.recycle_all: restores pool","[recv_bundle]"){
Rig rig{4,64};
...
TEST_CASE("recv_bundle.bundle: non-sequential recycled IDs","[recv_bundle]"){
Rig rig{4,64};
...
TEST_CASE("recv_bundle.discard: bundle head advances by cnt","[recv_bundle]"){
Rig rig{8,64};
...
TEST_CASE("recv_bundle.mixed: non-recv CQEs leave head unchanged","[recv_bundle]"){
Rig rig{8,64};
...
TEST_CASE("recv_bundle.no_buffer: positive res without buffer flag","[recv_bundle]"){
Rig rig{8,64};
u32 const before=rig.ring.debug_head_pos();
...
TEST_CASE("recv_bundle.scope_exit: recycles on exception","[recv_bundle]"){
Rig rig{4,64};
