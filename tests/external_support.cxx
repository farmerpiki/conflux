...
SV path)const{
return run_cmd_retry(format(
"curl -sk --http3-only --max-time 5 " "--resolve localhost:{}:127.0.0.1 https://localhost:{}{}",
port_,
port_,
path));
}
...
SV path)const{
return run_cmd_retry(format(
"curl -sk --http3-only -o /dev/null -w '%{{http_code}}' --max-time 5 " "--resolve localhost:{}:127.0.0.1 https://localhost:{}{}",
port_,
port_,
path));
}
