...
namespace{
Ring make_ring(unsigned sq=16){
auto r=Ring::init(sq,{});
REQUIRE(r);
return move(*r);
}
// Submit n NOP SQEs in batches of sq_size without draining the CQ.
void submit_nops_no_drain(Ring&ring,unsigned n,unsigned sq_size){
...
TEST_CASE("uring.cq_overflow_count: zero on fresh ring","[cq_overflow]"){
auto ring=make_ring();
CHECK(ring.cq_has_overflow()==false);
CHECK(ring.cq_overflow_count()==0u);
}
TEST_CASE("uring.RingRef.has_feature: consistent with Ring::has_feature","[cq_overflow]"){
auto ring=make_ring();
...
TEST_CASE("uring.RingRef.cq_overflow_count: matches Ring","[cq_overflow]"){
auto ring=make_ring();
RingRef ref=ring.ref();
CHECK(ring.cq_overflow_count()==ref.cq_overflow_count());
}
TEST_CASE("uring.cq_overflow_count: koverflow null-safe","[cq_overflow]"){
...
TEST_CASE("uring.cq_has_overflow: no overflow when CQ drained","[cq_overflow]"){
...
TEST_CASE("uring.cq_overflow: submit past CQ depth triggers overflow","[cq_overflow]"){
...
TEST_CASE("uring.cq_overflow_count: monotonically non-decreasing (without NODROP only)","[cq_overflow]"){
auto ring_exp=Ring::init(4,{});
REQUIRE(ring_exp);
auto ring=move(*ring_exp);
