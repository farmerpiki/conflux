...
TEST_CASE(
	"uring.ring_resize: build_has_io_uring_resize_rings constant accessible",
	"[ring_resize]") {
...
TEST_CASE(
	"uring.ring_resize: grow_cq_to returns -ENOSYS when built without symbol",
	"[ring_resize]") {
...
TEST_CASE(
	"uring.ring_resize: grow_cq_to grows CQ on DEFER_TASKRUN ring",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
...
TEST_CASE(
	"uring.ring_resize: resize({current_sq,current_cq}) succeeds as identity resize",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: grow_cq_to(current) is no-op success",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: grow_cq_to(smaller) is no-op success",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: resize blocked when CQ overflow active",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: resize fails without DEFER_TASKRUN",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: resize fails on NO_MMAP ring",
	"[ring_resize]") {
	if constexpr (!build_has_io_uring_resize_rings) {
		SKIP("built without io_uring_resize_rings");
	}
...
TEST_CASE(
	"uring.ring_resize: RingRef accessors match Ring entries",
	"[ring_resize]") {
