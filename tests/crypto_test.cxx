...
TEST_CASE(
	"crypto: base64_encode known vector",
	"[crypto]") {
	string_view const s = "Man";
...
TEST_CASE(
	...
	"[crypto]") {
	array<unsigned char, 2> raw{0xFB, 0xFF};
	auto enc = base64url_encode(raw);
	CHECK(enc.find('+') == string::npos);
	CHECK(enc.find('/') == string::npos);
	CHECK(enc.find('=') == string::npos);
}
...
TEST_CASE(
	...
	string_view const s = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";
	auto h = sha1({reinterpret_cast<unsigned char const *>(s.data()), s.size()});
	CHECK(h[0] == 0x84);
	CHECK(h[19] == 0xf1);
}
...
TEST_CASE(
	"crypto: hmac_sha256 rfc4231 test vector 1",
	"[crypto]") {
	// Key = 20 bytes of 0x0b, Data = "Hi There"
	// HMAC = b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7
	array<unsigned char, 20> key{};
	key.fill(0x0b);
	string_view const data = "Hi There";
...
TEST_CASE(
	"crypto: hmac_sha256 rfc4231 test vector 2",
	"[crypto]") {
	// Key = "Jefe", Data = "what do ya want for nothing?"
	// HMAC = 5bdcc146bf60754e6a042426089575c75a003f089d2739839dec58b964ec3843
	string_view const key = "Jefe";
	string_view const data = "what do ya want for nothing?";
	auto h = hmac_sha256(
...
TEST_CASE(
	...
	"[crypto]") {
	string_view const key = "secret";
	string_view const d1 = "message1";
	string_view const d2 = "message2";
	auto h1 = hmac_sha256(
		{reinterpret_cast<unsigned char const *>(key.data()), key.size()},
