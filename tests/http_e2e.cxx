...
std::call_once(flag,[]{
Router router;
router.use(redirect_middleware({.rules={
...
HttpClient client{};
auto response=
client.send_blocking(chttp::HttpRequest::get(format("http://127.0.0.1:{}/api/ping",g_test_port)));
REQUIRE(response);
CHECK(response->head.status==200);
CHECK(S{response->head.headers["content-type"]}=="application/json");
CHECK(response->body==R"({"status":"ok"})");
...
auto response=
client.send_blocking(chttp::HttpRequest::head(format("http://127.0.0.1:{}/api/ping",g_test_port)));
REQUIRE(response);
CHECK(response->head.status==200);
CHECK(S{response->head.headers["content-type"]}=="application/json");
...
chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now()-started).count();
...
auto resp=
http_get_with_header_on(g_jwt_port,"/api/protected",format("Authorization: Bearer {}\r\n",token));
REQUIRE(resp.starts_with("HTTP/1.1 200 OK"));
auto hdr_end=resp.find("\r\n\r\n");
REQUIRE(hdr_end!=S::npos);
...
auto resp=
http_get_with_header_on(port,"/api/protected/attacker",format("Authorization: Bearer {}\r\n",token));
REQUIRE(resp.starts_with("HTTP/1.1 200 OK"));
auto hdr_end=resp.find("\r\n\r\n");
REQUIRE(hdr_end!=S::npos);
...
Router router;
router.use(
response_cache_middleware({.max_entries=10,.max_bytes=16,.default_ttl=chrono::seconds{60}}));
router.get("/a",[&hits](HttpRequest const&){
++hits;
