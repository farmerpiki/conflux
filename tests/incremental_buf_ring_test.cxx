...
return{uring.ref(),BufferRingOptions{.count=count,.buf_size=buf_size,.group_id=gid,.huge_pages=false,.mode=BufferRingMode::incremental},caps};
}()}{}
};
}// namespace
// ─── Original tests ───────────────────────────────────────────────────────────
...
TEST_CASE("incremental: phase3 safety-net: cleared flags → bad_cqe, offset unchanged","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
...
TEST_CASE("incremental: append clears rc.flags for BUF_MORE: bad_cqe on re-call with flags=0","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
...
TEST_CASE("incremental: append final: head+1, RAII destructor recycles once","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
{
...
TEST_CASE("incremental: close after partial: reclaim_incremental_partial resets offset and recycles","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
...
TEST_CASE("incremental: no false recycle after final: reclaim_incremental_partial returns false when offset=0","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
...
TEST_CASE("incremental: BufferRing throws when feat_pbuf_ring_inc=false","[incremental]"){
auto r=conflux::uring::Ring::init(32,{});
REQUIRE(r);
...
TEST_CASE("incremental: skip when kernel lacks IORING_FEAT_PBUF_RING_INC","[incremental]"){
Rig rig{4,32};
...
TEST_CASE("incremental: old-gen tombstone reclaim [e2e required]","[incremental][.e2e]"){
...
TEST_CASE("incremental: partial-fill: two BUF_MORE CQEs accumulate offsets","[incremental]"){
Rig rig{8,64};
u32 const h0=rig.ring.debug_head_pos();
...
TEST_CASE("incremental: full-fill then next buffer: consume + offset reset","[incremental]"){
...
TEST_CASE("incremental: classic vs incremental same data same outcome","[incremental]"){
...
BufferRing classic_ring{r_classic->ref(),
BufferRingOptions{.count=8,.buf_size=64,.group_id=1,.huge_pages=false,.mode=BufferRingMode::classic_one_cqe_per_buffer},
any_caps};
...
TEST_CASE("incremental: WS handoff tombstone retire [e2e required]","[incremental][.e2e]"){
...
TEST_CASE("incremental: invalid CQE bounds → bad_bounds, no offset/head mutation","[incremental]"){
...
TEST_CASE("incremental: stale positive final CQE clears tombstone [e2e required]","[incremental][.e2e]"){
