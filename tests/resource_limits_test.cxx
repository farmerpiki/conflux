...
struct RingGuard{
io_uring ring{};
explicit RingGuard(unsigned entries){
...
TEST_CASE("resource: io_uring ring create/destroy does not leak fds","[resource]"){
...
TEST_CASE("resource: submit_fixed_fd_install does not leak OS fds","[resource]"){
...
TEST_CASE("resource: socket() returns EMFILE under fd exhaustion","[resource]"){
...
if(errno!=EMFILE&&errno!=ENFILE)::_exit(3);// wrong error
});
REQUIRE(rc==0);
}
TEST_CASE("resource: io_uring_queue_init fails under fd exhaustion","[resource]"){
int const rc=fork_run([]{
if(!seal_fds())::_exit(1);
io_uring ring{};
...
TEST_CASE("resource: DirectFdTable skips registration under fd exhaustion","[resource]"){
...
io_uring ring{};
if(::io_uring_queue_init(64,&ring,0)!=0)::_exit(0);// ring failed, skip
if(!seal_fds()){
::io_uring_queue_exit(&ring);
::_exit(1);
}
...
TEST_CASE("resource: BufferRing fails gracefully under low RLIMIT_MEMLOCK","[resource]"){
...
if(::setrlimit(RLIMIT_MEMLOCK,&rl)!=0)::_exit(1);
io_uring ring{};
