...
int main() {
	...
	bool all_ok = true;
	for (std::size_t sz: {0, 1, 15, 16, 17, 64, 256, 1024, 4096, 65536}) {
		...
		if (ossl_ct.size() != conflux_ct->size() || std::memcmp(ossl_ct.data(), conflux_ct->data(), ossl_ct.size()) != 0) {
			std::printf("  FAIL sz=%zu: ciphertext mismatch (ossl=%zu, conflux=%zu)\n", sz, ossl_ct.size(), conflux_ct->size());
			all_ok = false;
			continue;
		}
