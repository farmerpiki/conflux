...
void aes256_encrypt_block(
	...
	for (int round = 1; round <= 14; ++round) {
		...
		// AddRoundKey
		for (SZ i = 0; i < 16; ++i) {
			s[i] = static_cast<unsigned char>(s[i] ^ static_cast<unsigned char>(ek.rk[static_cast<SZ>(round) * 4 + i / 4] >> (24 - (i % 4) * 8)));
		}
	}
	ranges::copy(s, out);
}
...
void ghash_mult(
	...
	for (int i = 0; i < 128; ++i) {
		...
		for (int j = 15; j > 0; --j) {
			v[static_cast<SZ>(j)] = static_cast<unsigned char>((v[static_cast<SZ>(j)] >> 1) | (v[static_cast<SZ>(j) - 1] << 7));
		}
		v[0] >>= 1;
		if (carry) {
...
export expected<V<unsigned char>, S> aes_gcm_encrypt(
	...
	// Encrypt: CTR mode starting at J0+1
	...
	// Length block: [len(A) in bits || len(C) in bits] as big-endian u64s
