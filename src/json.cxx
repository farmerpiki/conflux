...
export enum class ParseMode:u8{strict,
json5};
...
u8 flags,
u64 raw)noexcept
:lexeme_{lex},raw_payload_{raw},flags_{flags}{}
public:
...
:raw_lexeme_{raw_lex},has_escapes_{has_esc},max_string_size_{max_sz}{}
public:
...
:storage_{s},mem_start_{start},mem_count_{count},node_idx_{node_idx}{}
public:
...
SZ start,
SZ count)noexcept
:storage_{s},child_start_{start},child_count_{count}{}
public:
...
SZ start,
SZ count)noexcept
:storage_{s},start_{start},count_{count}{}
public:
struct Sentinel{};
class Iterator{
DocumentStorage const*storage_{};
...
SZ cnt,
SZ i)noexcept
:storage_{s},start_{st},idx_{i},count_{cnt}{}
public:
...
return a.storage_->bytes_at(a.rec().off,a.rec().len,a.rec().flags)==b.storage_->bytes_at(b.rec().off,b.rec().len,b.rec().flags);
...
break;
if(est_bytes>0&&opts.max_extra_bytes!=SIZE_MAX&&bytes_allocated+est_bytes>opts.max_extra_bytes)
break;
...
:storage_{s},generation_{gen},arena_gen_{ag}{}
...
:initial_slab_{opts.initial_slab},mbr_{opts.initial_slab},storage_{make_unique<DocumentStorage>(&mbr_)}{}
...
struct PathFrame{
enum class Kind:u8{member,
index}kind;
SV member_name{};
SZ index{};
};
[[nodiscard]]inline JsonPath materialize_path(span<PathFrame const>frames){
JsonPath p;
...
export template<class T>concept has_json_codec=detail::has_codec_spec<T>::value||detail::has_members_spec<T>::value;
...
public:
ValueBuilder()
:owned_{make_unique<BuilderState>()},state_{owned_.get()}{}
ValueBuilder(
ValueBuilder&&o)noexcept
:owned_{move(o.owned_)},state_{owned_?owned_.get():o.state_}{
o.state_=nullptr;
}
...
return "boolean";
else if constexpr(same_as<Raw,i64>||same_as<Raw,u64>||same_as<Raw,i32>||same_as<Raw,u32>||same_as<Raw,i16>||same_as<Raw,u16>||same_as<Raw,i8>||same_as<Raw,u8>)
return "integer";
...
export template<class T>concept JsonWritable=
...
export template<class P>concept JsonObjectPair=
...
export template<class R>concept HandlerReturn=
same_as<R,void>||
std::convertible_to<R,expected<void,JsonError>>;

export template<class H>concept JsonHandler=
