...
public:
	...
	[[nodiscard]] bool try_push(Fn fn) noexcept {
		SZ pos = head_.load(std::memory_order_relaxed);
		for (;;) {
			Slot &slot = slots_[pos & mask_];
			SZ const seq = slot.seq.load(std::memory_order_acquire);
			auto const diff = static_cast<std::intptr_t>(seq) - static_cast<std::intptr_t>(pos);
			if (diff == 0) {
				if (head_.compare_exchange_weak(pos, pos + 1, std::memory_order_relaxed)) {
					slot.item = std::move(fn);
	...
	[[nodiscard]] Opt<Fn> try_pop() noexcept {
		SZ pos = tail_.load(std::memory_order_relaxed);
		for (;;) {
			Slot &slot = slots_[pos & mask_];
			SZ const seq = slot.seq.load(std::memory_order_acquire);
