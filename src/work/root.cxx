...
EP const&cause)
:WorkError{"work failed"},cause_{detail::normalize_failure_ptr(cause)}{}
...
CancelReason reason)
:WorkError{"work cancelled"},reason_{reason}{}
...
template<typename T>concept work_value=
...
Outcome()=delete;
Outcome(
success_t success)noexcept(std::is_nothrow_move_constructible_v<success_t>)
...
template<typename F>
requires std::invocable<F&,success_t&>&&std::invocable<F&,Failure&>&&std::invocable<F&,Cancelled&>&&same_as<std::invoke_result_t<F&,success_t&>,std::invoke_result_t<F&,Failure&>>&&same_as<std::invoke_result_t<F&,success_t&>,std::invoke_result_t<F&,Cancelled&>>
auto visit(
F&&f)
&noexcept(
std::is_nothrow_invocable_v<F&,success_t&>&&std::is_nothrow_invocable_v<F&,Failure&>&&std::is_nothrow_invocable_v<F&,Cancelled&>)->std::invoke_result_t<F&,success_t&>{
switch(kind()){
...
template<typename F>
requires std::invocable<F&,success_t const&>&&std::invocable<F&,Failure const&>&&std::invocable<F&,Cancelled const&>&&same_as<std::invoke_result_t<F&,success_t const&>,std::invoke_result_t<F&,Failure const&>>&&same_as<std::invoke_result_t<F&,success_t const&>,std::invoke_result_t<F&,Cancelled const&>>
auto visit(
F&&f)
const&noexcept(
std::is_nothrow_invocable_v<F&,success_t const&>&&std::is_nothrow_invocable_v<F&,Failure const&>&&std::is_nothrow_invocable_v<F&,Cancelled const&>)->std::invoke_result_t<F&,success_t const&>{
switch(kind()){
case OutcomeKind::success:return invoke(f,success());
case OutcomeKind::failure:return invoke(f,failure());
case OutcomeKind::cancelled:return invoke(f,cancelled());
...
requires std::invocable<OnSuccess,T&&>&&std::invocable<OnFailure,Failure const&>&&std::invocable<OnCancelled,Cancelled const&>&&same_as<std::invoke_result_t<OnSuccess,T&&>,std::invoke_result_t<OnFailure,Failure const&>>&&same_as<std::invoke_result_t<OnSuccess,T&&>,std::invoke_result_t<OnCancelled,Cancelled const&>>
auto match(
...
template<class OnSuccess,class OnFailure,class OnCancelled>
requires std::invocable<OnSuccess,T const&>&&std::invocable<OnFailure,Failure const&>&&std::invocable<OnCancelled,Cancelled const&>&&same_as<std::invoke_result_t<OnSuccess,T const&>,std::invoke_result_t<OnFailure,Failure const&>>&&same_as<std::invoke_result_t<OnSuccess,T const&>,std::invoke_result_t<OnCancelled,Cancelled const&>>
auto match(
OnSuccess&&on_success,
OnFailure&&on_failure,
...
template<class OnSuccess,class OnFailure,class OnCancelled>
requires std::invocable<OnSuccess>&&std::invocable<OnFailure,Failure const&>&&std::invocable<OnCancelled,Cancelled const&>&&same_as<std::invoke_result_t<OnSuccess>,std::invoke_result_t<OnFailure,Failure const&>>&&same_as<std::invoke_result_t<OnSuccess>,std::invoke_result_t<OnCancelled,Cancelled const&>>
auto match(
OnSuccess&&on_success,
OnFailure&&on_failure,
...
template<class C>concept progress_capability=requires(C const&c){
...
:LE{make_msg(r)},reason_{r},origin_{loc}{}
explicit JoinError(
reason r,
Opt<CapabilityId>expected,
CapabilityId actual,
std::source_location loc=std::source_location::current())
:LE{make_msg(r)},reason_{r},expected_{expected},actual_{actual},origin_{loc}{}
...
if constexpr(
sizeof(fn_t)<=InlineBytes&&alignof(fn_t)<=alignof(storage_t)&&std::is_nothrow_move_constructible_v<fn_t>){
object_=&inline_storage_;
...
.compare_exchange_strong(expected,true,memory_order_acq_rel,memory_order_acquire);
}
...
hook_fn_=move(fn);
if(!hook_claimed_&&cancel_requested_.load(memory_order_acquire)&&!terminal_claimed_.load(memory_order_acquire)){
hook_claimed_=true;
...
"P2b padding regressed vs P2a baseline beyond one cache line — " "define CONFLUX_WORK_RELAX_CONTROL_BLOCK_SIZE_GUARD to bypass");
...
"P2b padding regressed beyond per-platform budget — " "define CONFLUX_WORK_RELAX_CONTROL_BLOCK_SIZE_GUARD to bypass");
#endif
...
template<work_value U>
P<BasicControl<ControlCategory::posted>,BasicSource<U,ControlCategory::posted>>friend make_posted_control_source();
template<work_value U>
P<BasicControl<ControlCategory::operation>,BasicSource<U,ControlCategory::operation>>friend make_operation_control_source();
public:
...
template<work_value U>
P<class BasicResult<U,ControlCategory::task>,BasicSource<U,ControlCategory::task>>friend make_task_source(struct SubmitOptions,std::source_location);
template<work_value U,progress_capability Owner>
P<class BasicResult<U,ControlCategory::posted>,BasicSource<U,ControlCategory::posted>>friend make_posted_source(Owner&,struct PostOptions,std::source_location);
template<work_value U,progress_capability Driver>
P<class BasicResult<U,ControlCategory::operation>,BasicSource<U,ControlCategory::operation>>friend make_operation_source(Driver&,struct OperationOptions,std::source_location);
public:
BasicSource()=default;
...
template<class Sink,class T>concept abandon_sink=std::is_nothrow_move_constructible_v<Sink>&&(std::is_nothrow_invocable_v<Sink&,Outcome<T>const&>||(std::is_nothrow_invocable_v<Sink&,Failure const&>&&std::is_nothrow_invocable_v<Sink&,Cancelled const&>));
...
SP<detail::ControlBlockInterface<T>>state)noexcept
:state_{move(state)},state_js_{state_?join_state::joinable:join_state::empty}{}
...
template<work_value U>
P<BasicResult<U,ControlCategory::task>,TaskSource<U>>friend make_task_source(SubmitOptions,std::source_location);
template<work_value U,progress_capability Owner>
P<BasicResult<U,ControlCategory::posted>,PostedSource<U>>friend make_posted_source(Owner&,PostOptions,std::source_location);
template<work_value U,progress_capability Driver>
P<BasicResult<U,ControlCategory::operation>,OperationSource<U>>friend make_operation_source(Driver&,OperationOptions,std::source_location);
...
:state_{move(other.state_)},state_js_{exchange(other.state_js_,join_state::empty)}{}
...
:inner_{move(task)},origin_{origin}{}
...
SP<detail::ControlBlockInterface<T>>state)noexcept
:state_{move(state)},live_{static_cast<bool>(state_)}{}
...
:state_{move(other.state_)},live_{exchange(other.live_,false)}{}
...
R&&value,
Sink sink={})
:value_{std::move(value)},sink_{std::move(sink)},armed_{true}{}
scoped_abandon(
scoped_abandon&&other)noexcept
:value_{move(other.value_)},sink_{move(other.sink_)},armed_{exchange(other.armed_,false)}{}
