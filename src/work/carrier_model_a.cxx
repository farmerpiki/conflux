...
requires(!same_as<T,void>)&&std::invocable<Fn&,T>&&root::work_value<std::remove_cvref_t<std::invoke_result_t<Fn&,T>>>
auto map_outcome(
root::Outcome<T>out,
...
V<EP>causes)
:WorkError{"carrier: multiple failures"},causes_{move(causes)}{}
[[nodiscard]]V<EP>causes_owned()const{return causes_;}
[[nodiscard("span lifetime bound to *this — moves invalidate")]]span<EP const>
...
:outcome_{move(outcome)},kind_{kind}{}
Chain(
root::Outcome<T>outcome,
CarrierKind kind,
root::CapabilityId cap)noexcept
:outcome_{move(outcome)},kind_{kind},bound_cap_{cap}{}
...
template<class Fn>
requires(same_as<T,void>&&std::invocable<Fn&>&&root::work_value<std::remove_cvref_t<std::invoke_result_t<Fn&>>>)||(!same_as<T,void>&&std::invocable<Fn&,T>&&root::work_value<std::remove_cvref_t<std::invoke_result_t<Fn&,T>>>)
[[nodiscard]]auto then(
Fn&&fn)&&{
...
return Chain<R>{
root::Outcome<R>{root::Success<R>{invoke(forward<Fn>(fn))}},
kind_,
bound_cap_};
}
}else{
...
return Chain<R>{
root::Outcome<R>{
root::Success<R>{invoke(forward<Fn>(fn),move(outcome_).success().value)}},
kind_,
bound_cap_};
}
}else{
...
template<class Fn>
requires std::invocable<Fn&,EP>&&(same_as<std::remove_cvref_t<std::invoke_result_t<Fn&,EP>>,T>||same_as<std::remove_cvref_t<std::invoke_result_t<Fn&,EP>>,Chain<T>>)
...
return Chain<T>{
root::Outcome<T>{root::Success<T>{invoke(forward<Fn>(fn),ep)}},
kind_,
bound_cap_};
}
...
template<class Fn>
requires std::invocable<Fn&>&&(same_as<std::remove_cvref_t<std::invoke_result_t<Fn&>>,T>||same_as<std::remove_cvref_t<std::invoke_result_t<Fn&>>,Chain<T>>)
...
return Chain<T>{
root::Outcome<T>{root::Success<T>{invoke(forward<Fn>(fn))}},
kind_,
bound_cap_};
}
}catch(...){return Chain<T>{root::Outcome<T>{root::Failure{current_exception()}},kind_,bound_cap_};}
...
template<class Fn>
requires std::invocable<Fn&,root::Outcome<T>>&&same_as<std::remove_cvref_t<std::invoke_result_t<Fn&,root::Outcome<T>>>,T>
