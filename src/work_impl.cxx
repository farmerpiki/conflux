...
struct WorkPoolState {
	...
	explicit WorkPoolState(
		...
		if (inject_shards == 0) {
			inject_shards = options.max_inject_queue == 0 ?
								std::size_t{1} :
								std::max(std::size_t{1}, std::min(options.threads, options.max_inject_queue));
		}
		options.inject_queue_shards = inject_shards;
		std::size_t const inject_capacity_per_shard =
			options.max_inject_queue == 0 ? std::size_t{0} :
	...
	[[nodiscard]] std::size_t inject_shard_count() const noexcept { return inject_rings.size(); }
	[[nodiscard]] bool begin_no_stealing_admission() noexcept {
		...
		for (;;) {
			...
			if (no_stealing_admission
					.compare_exchange_weak(state, state + 1, std::memory_order_acq_rel, std::memory_order_acquire)) {
	...
	void end_no_stealing_admission() noexcept { no_stealing_admission.fetch_sub(1, std::memory_order_release); }
	...
	void discard_queued_jobs() noexcept { pending.store(0, std::memory_order_release); }
...
[[nodiscard]] static WorkPoolState *work_pool_state(
	void *state) noexcept {
	return static_cast<WorkPoolState *>(state);
}

[[nodiscard]] static RingLaneState *ring_lane_state(
	void *state) noexcept {
