...
std::optional<std::string> verify_cookie_with_validated_secrets(
...
std::expected<std::string, std::string> sign_cookie(
	std::string_view value,
	conflux::http::ResolvedSecretRotation const &secrets) {
	if (auto valid = conflux::http::validate_secret_rotation(secrets, "cookie"); !valid) {
...
std::optional<std::string> verify_cookie(
	...
	return cookie_signing_detail::verify_cookie_with_validated_secrets(signed_value, secrets);
}
...
Router::Middleware cookie_signing_middleware(
	...
	if (auto valid = conflux::http::validate_secret_rotation(opts.secrets, "cookie"); !valid) {
	...
	return [opts = std::move(opts)](
		...
		for (auto &[name, value]: modified.cookies) {
			...
			auto plain = cookie_signing_detail::verify_cookie_with_validated_secrets(value, opts.secrets);
			if (plain) {
