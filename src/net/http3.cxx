...
i64 qenc_id{};
i64 qdec_id{};
if(ngtcp2_conn_open_uni_stream(c->conn.get(),&ctrl_id,nullptr)!=0||ngtcp2_conn_open_uni_stream(c->conn.get(),&qenc_id,nullptr)!=0||ngtcp2_conn_open_uni_stream(c->conn.get(),&qdec_id,nullptr)!=0)
return NGTCP2_ERR_CALLBACK_FAILURE;
if(nghttp3_conn_bind_control_stream(c->h3conn.get(),ctrl_id)!=0||nghttp3_conn_bind_qpack_streams(c->h3conn.get(),qenc_id,qdec_id)!=0)
return NGTCP2_ERR_CALLBACK_FAILURE;
return 0;
}
void fill_callbacks(
...
u16 port,
SSL_CTX*ssl_ctx)
:router_(router),cfg_(cfg),port_(port),ssl_ctx_(ssl_ctx){}
~Http3Listener(){stop();}
...
u64 const ms=delta/1000000ULL;
return static_cast<int>(min<u64>(ms,1000ULL));
}
...
u32 const flags=(sveccnt>0?NGTCP2_WRITE_STREAM_FLAG_MORE:NGTCP2_WRITE_STREAM_FLAG_NONE)|(fin!=0?NGTCP2_WRITE_STREAM_FLAG_FIN:0u);
