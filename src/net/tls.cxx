...
:ssl_{move(other.ssl_)},fd_{exchange(other.fd_,-1)}{}
TlsStream&operator=(
TlsStream&&other)noexcept{
if(this!=&other){
...
FileReader&files,
FileHandle sock)
:ssl_{SSL_new(ctx.native_handle())},files_{&files},sock_{move(sock)}{
if(!ssl_)
...
:ssl_{move(other.ssl_)},rbio_{exchange(other.rbio_,nullptr)},wbio_{exchange(other.wbio_,nullptr)},files_{exchange(other.files_,nullptr)},sock_{move(other.sock_)},scratch_{other.scratch_}{}
...
UniqueBio rbio{BIO_new(BIO_s_mem())};
UniqueBio wbio{BIO_new(BIO_s_mem())};
if(!rbio||!wbio)throw TlsError{"TcpTlsStream: BIO_new failed"};
rbio_=rbio.get();
wbio_=wbio.get();
...
bool set_server_name(SV sni){
if(sni.empty())return true;
S const s{sni};
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wold-style-cast"
return SSL_set_tlsext_host_name(ssl_.get(),s.c_str())==1;
...
bool set_verify_hostname(SV host){
...
int const err=SSL_get_error(ssl_.get(),n);
if(err==SSL_ERROR_ZERO_RETURN)co_return SZ{0};
if(err==SSL_ERROR_WANT_READ){
co_await fill_rbio(deadline);
continue;
}
