...
std::string zlib_ng_gzip_compress(
	...
	zs.next_in = reinterpret_cast<std::uint8_t const *>(input.data());
	zs.avail_in = static_cast<std::uint32_t>(input.size());

	std::string out(zng_deflateBound(&zs, input.size()), '\0');
	zs.next_out = reinterpret_cast<std::uint8_t *>(out.data());
	zs.avail_out = static_cast<std::uint32_t>(out.size());
