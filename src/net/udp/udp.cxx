...
auto shared_src=make_shared<TaskSource<UdpRecvResult>>(move(src));
auto shared_h=h;
...
try{
auto outcome=join(move(*shared_jh));
...
(void)shared_src->try_set_exception(move(outcome).failure().error);
...
auto shared_src=make_shared<TaskSource<UdpRecvResult>>(move(raw_src));
