...
export struct ActiveTaskCancelRelay {
	...
	[[nodiscard]] bool is_cancelled() const noexcept { return cancelled_.load(memory_order_acquire); }
	void throw_if_cancelled() const {
		if (is_cancelled()) {
			throw wroot::CancelledError{wroot::CancelReason::requested};
		}
	}
	[[nodiscard]] wroot::Task<SZ> await_child(
		wroot::Task<SZ> child) {
	...
	[[nodiscard]] wroot::Task<void> await_child(
