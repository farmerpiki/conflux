...
|---|---|---|---|---|
| P0 | `build/bench-regression-budget` | Yes. This is the highest-leverage non-component gap because the perf harness exists but deltas are still advisory. | `scripts/bench_record.sh`, `scripts/bench_db_migrate.sql`, `benchmarks/README.md`, optional new `scripts/bench_check_budget.py` / CTest guard. | Same-machine baseline/candidate comparison has explicit per-benchmark budgets, noisy rows are classified, and merge-blocking output points to recorded artifact paths. |
...
| P1 | `build/stale-tcp-parallel-bench-prune` | Yes if we prefer deletion over resurrection. The standalone bench is disabled and the intended N=4 coverage now lives in `tcp_increment_coro_bench`. | `benchmarks/CMakeLists.txt`, `benchmarks/README.md`, `scripts/bench_record.sh`, possibly delete `benchmarks/tcp_parallel_coro_bench.cxx`. | No stale `co_spawn` bench TODO remains; recorder docs/scripts do not carry an unused `tcp_parallel` parser unless a live target still emits it. |
| P2 | `worker/queue-contention-measurement` | Yes, measurement-only. Do not replace local deque/admission locks without counter evidence. | `benchmarks/notes/worker_queue_contention_profile.md`, benchmark run artifacts, optional docs update. | `CONFLUX_WORK_QUEUE_STATS=ON` runs show admission/local/steal contention under burst/fanout loads and either justify a follow-up or record a no-change decision. |
...
4. Prune or port the disabled standalone TCP parallel bench.
5. Only after recorded evidence, decide whether worker lock replacement or
