...
1. **Keep file-layer boundaries honest.**
   ...
   - Next large-module split candidate outside HTTP/JSON: `src/file_io/file_io.cxx`; see `docs/archive/proposals/README.md`. Keep the first implementation branch source-only inside the existing `conflux_file_io` target.
