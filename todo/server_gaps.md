...
- [ ] HTTP send path edge cases: mapped-file header+body now splits header send from large-body SEND_ZC, but TLS path cannot use SEND_ZC directly and adaptive threshold still needs benchmark validation. Note: mapped cache (when landed) removes repeated open/stat/mmap/close/munmap for hot static files but does not change the send path itself
