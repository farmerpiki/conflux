...
- Release-gated: ring-thread `sched_setaffinity` and `IORING_REGISTER_IOWQ_AFF` application:
  add syscall injection or observable thread/affinity capture if this becomes a
  release gate.
- Deferred: HTTP streaming upload API: add a bounded-memory request body/multipart
...
- [x] Keep HTTP handler execution docs aligned with code: handlers run on ring
  threads unless users explicitly offload work.
- [x] Do not add hidden sync-handler auto-offload.
