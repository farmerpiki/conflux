...
- [ ] Remove/deprecate `FileReader` socket methods after HTTP/DNS migration complete. The stale `socket_io` → `file_io` CMake PUBLIC link has been removed; remaining `FileReader` socket users are explicit compatibility/benchmark paths.
