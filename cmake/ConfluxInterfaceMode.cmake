...
function(conflux_apply_header_generated_build_policy target)
    cmake_parse_arguments(_AHGBP "FORCE_FAST" "" "" ${ARGN})
    ...
    if(CONFLUX_HEADER_FAST_COMPILE OR _AHGBP_FORCE_FAST)
...
function(conflux_register_header_public_profile_hpps)
    if(NOT umbrella IN_LIST CONFLUX_PACKAGE_COMPONENTS)
        return()
    endif()
    foreach(_profile IN ITEMS curated extended complete)
        conflux_register_header_public_hpp("${_profile}")
    endforeach()
endfunction()
...
function(conflux_define_header_impl_component_by_target target)
    foreach(_entry IN LISTS CONFLUX_HEADER_IMPL_DECLARATIONS)
        string(REPLACE "|" ";" _parts "${_entry}")
        list(GET _parts 0 _declared_target)
        if(NOT _declared_target STREQUAL target)
            continue()
        endif()
        list(GET _parts 1 _export_name)
        list(REMOVE_AT _parts 0 1)
        string(JOIN "|" _module_regex ${_parts})
        conflux_define_header_impl_component(
            ${target} ${_export_name} "${_module_regex}")
        return()
    endforeach()
    message(FATAL_ERROR
        "conflux: unknown header implementation target '${target}'")
endfunction()
...
function(conflux_define_header_impl_targets)
    ...
    conflux_define_header_impl_component_by_target(conflux_header_impl_core)

    if(CONFLUX_WANT_JSON OR CONFLUX_WANT_HTTP_JSON OR CONFLUX_WANT_HTTP_SERVER)
        conflux_define_header_impl_component_by_target(conflux_header_impl_json)
    ...
    if(CONFLUX_NEEDS_RUNTIME)
        conflux_define_header_impl_component_by_target(conflux_header_impl_runtime)
    ...
    if(CONFLUX_EFFECTIVE_FILE_IO_SYNC)
        conflux_define_header_impl_component_by_target(conflux_header_impl_file_io_sync)
    endif()
    if(CONFLUX_WANT_FILE_MAP)
        conflux_define_header_impl_component_by_target(conflux_header_impl_file_map)
    endif()
    if(CONFLUX_WANT_FILE_IO)
        conflux_define_header_impl_component_by_target(conflux_header_impl_file_io)
    ...
    if(CONFLUX_WANT_HTTP_CORE OR CONFLUX_WANT_HTTP_JSON OR CONFLUX_WANT_HTTP_SERVER)
        conflux_define_header_impl_component_by_target(conflux_header_impl_http_core)
    ...
    if(CONFLUX_WANT_HTTP_SERVER)
        conflux_define_header_impl_component_by_target(conflux_header_impl_http_server)
    ...
    if(CONFLUX_WANT_HTTP_STATIC OR CONFLUX_HTTP_ROUTER_STACK_REQUESTED OR CONFLUX_WANT_HTTP_SERVER)
        conflux_define_header_impl_component_by_target(conflux_header_impl_http_static)
    ...
    if(CONFLUX_WANT_HTTP_CLIENT OR CONFLUX_HTTP_CLIENT_STACK_REQUESTED)
        conflux_define_header_impl_component_by_target(conflux_header_impl_http_client)
    ...
    if(CONFLUX_WANT_HTTP_PROXY)
        conflux_define_header_impl_component_by_target(conflux_header_impl_http_proxy)
    ...
    if(CONFLUX_WANT_TEMPLATES)
        conflux_define_header_impl_component_by_target(conflux_header_impl_templates)
    endif()
    if(CONFLUX_HAS_DB STREQUAL "true")
        conflux_define_header_impl_component_by_target(conflux_header_impl_pg)
    ...
    if(CONFLUX_EFFECTIVE_SMTP)
        conflux_define_header_impl_component_by_target(conflux_header_impl_smtp)
...
int main() {
    ...
    conflux_apply_header_generated_build_policy(conflux_header_link_smoke_http FORCE_FAST)
...
function(conflux_add_header_component_smoke_targets)
    ...
    if(CONFLUX_PUBLIC_HEADER_SMOKE_SOURCES)
        ...
        conflux_apply_header_generated_build_policy(conflux_header_smoke_public_includes FORCE_FAST)
    ...
    conflux_apply_header_generated_build_policy(conflux_header_smoke_core FORCE_FAST)
    ...
    if(CONFLUX_WANT_HTTP_SERVER)
        foreach(_surface IN ITEMS curated extended complete selected)
            if(_surface STREQUAL "selected")
                ...
                set(_surface_level 1)
                if(_surface STREQUAL "extended")
                    set(_surface_level 2)
                elseif(_surface STREQUAL "complete")
                    set(_surface_level 3)
                endif()
                if(_surface_level GREATER CONFLUX_API_SURFACE_LEVEL_VALUE)
                    continue()
                endif()
            ...
            conflux_apply_header_generated_build_policy(${_surface_target} FORCE_FAST)
    ...
    if(CONFLUX_WANT_JSON)
        ...
        conflux_apply_header_generated_build_policy(conflux_header_smoke_json FORCE_FAST)
    ...
    if(CONFLUX_NEEDS_RUNTIME)
        ...
        conflux_apply_header_generated_build_policy(conflux_header_smoke_runtime FORCE_FAST)
    ...
    if(CONFLUX_HEADER_INSTALL_DB_COMPONENTS)
        ...
        conflux_apply_header_generated_build_policy(conflux_header_smoke_pg FORCE_FAST)
