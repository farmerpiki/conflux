...
function(conflux_apply_api_surface_definitions target scope)
    ...
    conflux_set_api_surface_presence(http_auth "$<AND:$<TARGET_EXISTS:conflux_http_auth>,$<BOOL:${CONFLUX_WANT_HTTP_AUTH}>>" "$<BOOL:${CONFLUX_WANT_HTTP_AUTH}>")
    conflux_set_api_surface_presence(http_policy "$<AND:$<TARGET_EXISTS:conflux_http_policy>,$<BOOL:${CONFLUX_WANT_HTTP_POLICY}>>" "$<BOOL:${CONFLUX_WANT_HTTP_POLICY}>")
    conflux_set_api_surface_presence(http_observability "$<AND:$<TARGET_EXISTS:conflux_http_observability>,$<BOOL:${CONFLUX_WANT_HTTP_OBSERVABILITY}>>" "$<BOOL:${CONFLUX_WANT_HTTP_OBSERVABILITY}>")
    conflux_set_api_surface_presence(http_openapi "$<AND:$<TARGET_EXISTS:conflux_http_openapi>,$<BOOL:${CONFLUX_WANT_HTTP_OPENAPI}>>" "$<BOOL:${CONFLUX_WANT_HTTP_OPENAPI}>")
    conflux_set_api_surface_presence(http_vhost "$<AND:$<TARGET_EXISTS:conflux_http_vhost>,$<BOOL:${CONFLUX_WANT_HTTP_VHOST}>>" "$<BOOL:${CONFLUX_WANT_HTTP_VHOST}>")
    conflux_set_api_surface_presence(http_metrics "$<AND:$<TARGET_EXISTS:conflux_http_observability>,$<BOOL:${CONFLUX_WANT_HTTP_OBSERVABILITY}>,$<BOOL:${CONFLUX_HAS_METRICS}>>" "$<AND:$<BOOL:${CONFLUX_WANT_HTTP_OBSERVABILITY}>,$<BOOL:${CONFLUX_HAS_METRICS}>>")
    ...
    conflux_set_api_surface_presence(http_static "$<AND:$<TARGET_EXISTS:conflux_http_static>,$<BOOL:${CONFLUX_HTTP_STATIC_SURFACE_REQUESTED}>>" "$<BOOL:${CONFLUX_HTTP_STATIC_SURFACE_REQUESTED}>")
    conflux_set_api_surface_presence(http_realtime "$<AND:$<TARGET_EXISTS:conflux_http_realtime>,$<BOOL:${CONFLUX_HTTP_REALTIME_SURFACE_REQUESTED}>>" "$<BOOL:${CONFLUX_HTTP_REALTIME_SURFACE_REQUESTED}>")
    conflux_set_api_surface_presence(http_server "$<TARGET_EXISTS:conflux_http_server>" "$<BOOL:${CONFLUX_WANT_HTTP_SERVER}>")
    conflux_set_api_surface_presence(http_compression "$<AND:$<TARGET_EXISTS:conflux_http_compression>,$<BOOL:${CONFLUX_WANT_HTTP_COMPRESSION}>>" "$<BOOL:${CONFLUX_WANT_HTTP_COMPRESSION}>")
    conflux_set_api_surface_presence(http_proxy "$<AND:$<TARGET_EXISTS:conflux_http_proxy>,$<BOOL:${CONFLUX_WANT_HTTP_PROXY}>>" "$<BOOL:${CONFLUX_WANT_HTTP_PROXY}>")
    ...
    conflux_set_api_surface_presence(work_uring_executor "$<TARGET_EXISTS:conflux_work_uring_executor>" "$<BOOL:${CONFLUX_WANT_RUNTIME}>")
    ...
    conflux_set_api_surface_presence(file_io_sync "$<TARGET_EXISTS:conflux_file_io_sync>" "$<BOOL:${CONFLUX_EFFECTIVE_FILE_IO_SYNC}>")
...
elseif(CONFLUX_JSON_REFLECT AND CMAKE_CXX_STANDARD LESS 26)
    message(FATAL_ERROR
        "conflux: CONFLUX_JSON_REFLECT requires C++26 or newer; got CMAKE_CXX_STANDARD=${CMAKE_CXX_STANDARD}.")
endif()
...
set(CONFLUX_RESOLVED_GZIP_PROVIDER "OFF" CACHE STRING
    "Resolved gzip backend for this build")
