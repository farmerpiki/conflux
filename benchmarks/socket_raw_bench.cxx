...
[[nodiscard]]int connect_one(u16 port){
int fd=::socket(AF_INET,SOCK_STREAM|SOCK_CLOEXEC,0);
if(fd<0)
throw RE{"socket"};
int one=1;
...
void wait_cqe(io_uring*ring,io_uring_cqe**out){
if(io_uring_wait_cqe(ring,out)!=0)
throw RE{"io_uring_wait_cqe"};
}
struct RingGuard{
io_uring ring{};
explicit RingGuard(unsigned entries,unsigned flags=0){
io_uring_params p{};
p.flags=flags;
...
FdGuard(FdGuard&&o)noexcept:fd{exchange(o.fd,-1)}{}
...
raw,SocketHandle::from_os(servers[i].fd),bufs,100+i);
raw.submit();
},
.teardown=
[&]{
