...
void print_variant(
	...
	if (json) {
		...
		auto const ns_per_op = s.ns_per_iter / static_cast<double>(ops_per_iter);
		println(
			"{:<40} {:>8} iters  {:>10.2f} ns/iter  {:>10.2f} ns/op (x{})",
			s.variant,
			s.iterations,
			s.ns_per_iter,
			ns_per_op,
		...
		println("{:<40} {:>8} iters  {:>10.2f} ns/iter", s.variant, s.iterations, s.ns_per_iter);
	}
}
...
int main(
	...
	auto make_router = [&] {
		...
		for (auto const &[label, body]: body_map) {
			r.get(format("/body/{}", label), [&body](HttpRequest const &) { return HttpResponse::text(body); });
		}
		return r;
	};
	...
	for (auto const &[label, size]: kBodies) {
		...
		variants.push_back(
			{.name = format("plain/{}/zc_auto", label_s),
	...
	for (auto const &[label, size]: kBodies) {
		...
		variants.push_back(
			{.name = format("mapped/{}/off", label_s),
		...
		variants.push_back(
			{.name = format("mapped/{}/zc_auto", label_s),
