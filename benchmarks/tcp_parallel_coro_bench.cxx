...
SZ iterations=20000;// per connection
SZ warmup=500;
bool json_out=false;
S config_path;
};
...
println(
"Usage: conflux_tcp_parallel_coro_bench [--iterations N] [--warmup N] " "[--parallel 1,2,4,8,16] [--config path.json] [--json]");
std::exit(0);
}
}
return cfg;
...
struct Barrier{
Atom<SZ>remaining;
SP<conflux::work::root::TaskSource<void>>src;
...
src=make_shared<conflux::work::root::TaskSource<void>>(move(source));
}
};
...
for(SZ i=0;i<parallelism;++i){
co_spawn(async_server(files,move(server_socks[i]),&b));
co_spawn(spawn_arm(worker(files,move(client_socks[i]),iters_per_conn,&b)));
}
block_on(files,move(b.done));
