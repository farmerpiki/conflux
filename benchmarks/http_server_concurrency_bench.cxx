...
[[nodiscard]]unsigned nproc(){return thread::hardware_concurrency();}
void pin_thread(unsigned cpu){
cpu_set_t set;
CPU_ZERO(&set);
CPU_SET(cpu,&set);
...
[[nodiscard]]SZ recv_response(span<char>buf)const{
SZ total=0;
...
[[nodiscard]]LatencyStats compute_percentiles(V<u64>&latencies){
...
long rss_start=-1,
long rss_end=-1){
double const req_per_sec=static_cast<double>(total_requests)/
(static_cast<double>(total_ns)/1e9);
double const ns_per_iter=total_requests>0?
static_cast<double>(total_ns)/static_cast<double>(total_requests):
0.0;

if(json){
S line=format(
"{{\"config\":\"{}\",\"variant\":\"{}\",\"iterations\":{},\"total_ns\":{},\"ns_per_iter\":{:.2f}" ",\"connections\":{},\"duration_s\":{},\"total_requests\":{},\"requests_per_sec\":{:.1f}" ",\"p50_ns\":{},\"p90_ns\":{},\"p99_ns\":{},\"p999_ns\":{},\"max_ns\":{}" ",\"errors\":{},\"pinned\":{}",
config,variant,total_requests,total_ns,ns_per_iter,
connections,duration_s,total_requests,req_per_sec,
lat.p50,lat.p90,lat.p99,lat.p999,lat.max,
errors,pinned?"true":"false");
if(fd_start>=0)
line+=format(",\"fd_count_start\":{},\"fd_count_end\":{},\"rss_kb_start\":{},\"rss_kb_end\":{}",
fd_start,fd_end,rss_start,rss_end);
line+="}";
println("{}",line);
}else{
println("{:<36} {:>8} reqs  {:>10.0f} req/s  p50={:>7}  p99={:>7}  p999={:>7}  max={:>7} ns",
variant,total_requests,req_per_sec,lat.p50,lat.p99,lat.p999,lat.max);
if(fd_start>=0)
...
auto make_router=[&]{
Router r;
r.get("/api/ping",[](HttpRequest const&){
...
{.name="parallel_connect_close_256"sv,.connections=256,.duration=chrono::seconds{duration_s},.mode=Mode::ConnectClose},
{.name="parallel_connect_close_1k"sv,.connections=1024,.duration=chrono::seconds{duration_s},.mode=Mode::ConnectClose},
{.name="parallel_post_4k_256"sv,.connections=256,.duration=chrono::seconds{duration_s},.request=SV{post_4k}},
{.name="parallel_mixed_256"sv,.connections=256,.duration=chrono::seconds{duration_s},.mixed=true},
{.name="parallel_large_body_64"sv,.connections=64,.duration=chrono::seconds{duration_s},.request=kGetBody64k},
{.name="idle_keepalive_1k"sv,.connections=1024,.duration=chrono::seconds{duration_s},.is_idle=true},
};

if(duration_s>=30){
variants.push_back({.name="stress_keepalive_256"sv,.connections=256,.duration=chrono::seconds{60},.is_stress=true});
variants.push_back({.name="stress_connect_close_256"sv,.connections=256,.duration=chrono::seconds{60},.mode=Mode::ConnectClose,.is_stress=true});
}
// ── Run ────────────────────────────────────────────────────────────
...
A<ServerConfig,2>configs{{{.suffix="_r1"sv,.port=r1.port},{.suffix="_rN"sv,.port=rn.port}}};
...
auto const num_threads=min(static_cast<int>(v.connections),can_pin?static_cast<int>(half):static_cast<int>(np));
...
emit_result(config_name,variant_name,total_requests,total_ns,
v.connections,static_cast<int>(dur.count()),lat,total_errors,can_pin,json,
fd_start,fd_end,rss_start,rss_end);
}
}
