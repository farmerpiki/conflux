...
SZ recv_response_no_body(span<char>buf)const{
SZ total=0;
for(;;){
auto n=::recv(fd,buf.data()+total,buf.size()-total,0);
if(n<=0)break;
...
SZ recv_n_responses(span<char>buf,int count)const{
SZ total=0;
int got=0;
SZ search_from=0;
while(got<count){
...
SZ recv_until_close(span<char>buf)const{
SZ total=0;
for(;;){
auto n=::recv(fd,buf.data()+total,buf.size()-total,0);
if(n<=0)break;
...
void send_partial(SV data,SZ bytes)const{
auto n=::send(fd,data.data(),min(bytes,data.size()),MSG_NOSIGNAL);
(void)n;
}
SZ recv_slow(span<char>buf,SZ chunk_size,chrono::microseconds delay)const{
SZ total=0;
SZ hdr_end_pos=SV::npos;
SZ body_len=0;
bool have_cl=false;
...
if(have_cl&&total>=hdr_end_pos+body_len)return total;
if(!have_cl&&hdr_end_pos!=SV::npos)return total;
std::this_thread::sleep_for(delay);
}
return total;
}
void shutdown_wr()const{
::shutdown(fd,SHUT_WR);
}
...
void set_recv_timeout(chrono::seconds t)const{
...
Config bench_config(unsigned rings=1,unsigned ring_entries=256){
Config cfg{};
cfg.port=0;
cfg.rings=rings;
...
static auto const kGetAuthBearer="GET /protected HTTP/1.1\r\nHost: localhost\r\nAuthorization: Bearer valid-bench-token\r\n\r\n"sv;
...
static auto const kGetFullStack="GET /big HTTP/1.1\r\nHost: localhost\r\nAccept-Encoding: gzip\r\nOrigin: https://bench.example\r\n\r\n"sv;
static auto const kGetRootClose="GET / HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n"sv;
S make_post_request(SV path,SZ body_size){
S body(body_size,'X');
return format(
"POST {} HTTP/1.1\r\nHost: localhost\r\nContent-Type: text/plain\r\nContent-Length: {}\r\n\r\n{}",
path,body_size,body);
}
S make_chunked_single(SV path,SZ body_size){
S body(body_size,'X');
return format(
"POST {} HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n{:x}\r\n{}\r\n0\r\n\r\n",
path,body_size,body);
}
S make_chunked_many(SV path,SZ total_size,SZ chunk_size){
...
static auto const kDuplicateCL="POST /api/echo-body HTTP/1.1\r\nHost: localhost\r\nContent-Length: 5\r\nContent-Length: 5\r\n\r\nhello"sv;
static auto const kConflictingCL="POST /api/echo-body HTTP/1.1\r\nHost: localhost\r\nContent-Length: 5\r\nContent-Length: 10\r\n\r\nhello"sv;
static auto const kTEPlusCL="POST /api/echo-body HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\nContent-Length: 5\r\n\r\n5\r\nhello\r\n0\r\n\r\n"sv;
static auto const kObsFold="GET / HTTP/1.1\r\nHost: localhost\r\nX-Test: value\r\n continued\r\n\r\n"sv;
static auto const kInvalidHeaderWS="GET / HTTP/1.1\r\nHost: localhost\r\nX\t-Bad: value\r\n\r\n"sv;
static auto const kInvalidTE="POST /api/echo-body HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: gzip\r\n\r\ndata"sv;
...
auto const ns_per_op=s.ns_per_iter/static_cast<double>(ops_per_iter);
println("{:<32} {:>8} iters  {:>10.2f} ns/iter  {:>10.2f} ns/op (x{})",
s.variant,s.iterations,s.ns_per_iter,ns_per_op,ops_per_iter);
}else{
println("{:<32} {:>8} iters  {:>10.2f} ns/iter",
...
plain_router.get("/api/ping",[](HttpRequest const&){
...
plain_router.post("/api/echo-body",[](HttpRequest const&req){
...
g.get("/status",[](HttpRequest const&){
return HttpResponse::json(R"({"v":"2","status":"ok"})");
});
});
...
for(;;){
auto frame=ws.recv();
if(!frame||frame->opcode==WsConn::Opcode::Close)break;
...
return conflux::http::defer(defer_pool,[]{
return HttpResponse::json(R"({"deferred":"ok"})");
});
});
...
rn_router.get("/api/ping",[](HttpRequest const&){
return HttpResponse::json(R"({"status":"ok"})");
});
rn_router.post("/api/echo-body",[](HttpRequest const&req){
return HttpResponse::text(req.body);
});
...
tls_router.get("/api/ping",[](HttpRequest const&){
return HttpResponse::json(R"({"status":"ok"})");
});
tls_router.post("/api/echo-body",[](HttpRequest const&req){
return HttpResponse::text(req.body);
});
...
variants.push_back({.name="get_html"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetRoot);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="get_json"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetJson);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="get_param"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetParam);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="post_echo_64"sv,
.setup=plain_setup,
.run=[&]{client->send_all(post_64);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="post_echo_4k"sv,
.setup=plain_setup,
.run=[&]{client->send_all(post_4k);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="post_echo_64k"sv,
.setup=plain_setup,
.run=[&]{client->send_all(post_64k);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="get_group"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetGroup);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="get_404"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGet404);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="keepalive_10"sv,
.setup=plain_setup,
.run=[&]{for(int i=0;i<10;++i){client->send_all(kGetJson);(void)client->recv_response(sb);}},
.teardown=plain_teardown,
.ops_per_iter=10});
variants.push_back({.name="connect_close"sv,
.setup={},
.run=[&]{BenchClient c{plain.port};c.send_all(kGetRoot);(void)c.recv_response(sb);},
.teardown={}});

// Large bodies
variants.push_back({.name="get_body_8k"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetBody8k);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="get_body_64k"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetBody64k);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="get_body_1m"sv,
.setup=plain_setup,
.run=[&]{client->send_all(kGetBody1m);(void)client->recv_response(lb);},
.teardown=plain_teardown});

// Pipelining
variants.push_back({.name="pipeline_10"sv,
.setup=plain_setup,
.run=[&]{
for(int i=0;i<10;++i)
...
.teardown=plain_teardown,
.ops_per_iter=10});
variants.push_back({.name="pipeline_100"sv,
.setup=plain_setup,
.run=[&]{
for(int i=0;i<100;++i)
client->send_all(kGetJson);
...
// Middleware — compress
variants.push_back({.name="mw_compress_gzip"sv,
.setup=[&]{client=make_unique<BenchClient>(compress.port);},
.run=[&]{client->send_all(kGetBigGzip);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="mw_compress_passthrough"sv,
.setup=[&]{client=make_unique<BenchClient>(compress.port);},
.run=[&]{client->send_all(kGetBigNoEnc);(void)client->recv_response(sb);},
.teardown=plain_teardown});

// security
variants.push_back({.name="mw_security"sv,
.setup=[&]{client=make_unique<BenchClient>(security.port);},
.run=[&]{client->send_all(kGetSecurity);(void)client->recv_response(sb);},
.teardown=plain_teardown});

// cors
variants.push_back({.name="mw_cors_origin"sv,
.setup=[&]{client=make_unique<BenchClient>(cors.port);},
.run=[&]{client->send_all(kGetCorsOrigin);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="mw_cors_passthrough"sv,
.setup=[&]{client=make_unique<BenchClient>(cors.port);},
.run=[&]{client->send_all(kGetCorsNone);(void)client->recv_response(sb);},
.teardown=plain_teardown});

// auth
variants.push_back({.name="mw_auth_bearer"sv,
.setup=[&]{client=make_unique<BenchClient>(auth.port);},
.run=[&]{client->send_all(kGetAuthBearer);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="mw_auth_reject"sv,
.setup=[&]{client=make_unique<BenchClient>(auth.port);},
.run=[&]{client->send_all(kGetAuthMissing);(void)client->recv_response(sb);},
.teardown=plain_teardown});

// etag
variants.push_back({.name="mw_etag_miss"sv,
.setup=[&]{client=make_unique<BenchClient>(etag.port);},
.run=[&]{client->send_all(kGetEtag);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="mw_etag_hit"sv,
.setup=[&]{client=make_unique<BenchClient>(etag.port);},
.run=[&]{client->send_all(kGetEtagHit);(void)client->recv_response_no_body(sb);},
.teardown=plain_teardown});

// cache
variants.push_back({.name="mw_cache_hit"sv,
.setup=[&]{client=make_unique<BenchClient>(cache.port);},
.run=[&]{client->send_all(kGetCacheHit);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="mw_cache_miss"sv,
.setup=[&]{client=make_unique<BenchClient>(cache.port);},
.run=[&]{
auto req=format("GET /counted?v={} HTTP/1.1\r\nHost: localhost\r\n\r\n",
...
// full_stack
variants.push_back({.name="mw_full_stack"sv,
.setup=[&]{client=make_unique<BenchClient>(full_stack.port);},
.run=[&]{client->send_all(kGetFullStack);(void)client->recv_response(sb);},
.teardown=plain_teardown});
...
auto const chunked_split_hdr="POST /api/echo-body HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n"s;
auto const chunked_split_c1="20\r\n"s+S(32,'X')+"\r\n";
auto const chunked_split_c2="20\r\n"s+S(32,'Y')+"\r\n";
auto const chunked_split_end="0\r\n\r\n"s;

variants.push_back({.name="post_chunked_64"sv,
.setup=plain_setup,
.run=[&]{client->send_all(chunked_64);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="post_chunked_4k_many"sv,
.setup=plain_setup,
.run=[&]{client->send_all(chunked_4k_many);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="post_chunked_split"sv,
.setup=plain_setup,
.run=[&]{
...
variants.push_back({.name="bad_request_line"sv,
.setup=error_setup,
.run=error_run_reconnect(kBadRequestLine),
.teardown=plain_teardown});
variants.push_back({.name="header_too_large"sv,
.setup=error_setup,
.run=error_run_reconnect(header_too_large),
.teardown=plain_teardown});
variants.push_back({.name="body_too_large"sv,
.setup=error_setup,
.run=[&]{
...
.teardown=plain_teardown});
variants.push_back({.name="connection_close"sv,
.setup=error_setup,
.run=error_run_reconnect(kConnectionClose),
.teardown=plain_teardown});
variants.push_back({.name="http_10_close"sv,
.setup=error_setup,
.run=error_run_reconnect(kHttp10Close),
.teardown=plain_teardown});
variants.push_back({.name="client_early_close"sv,
.setup={},
.run=[&]{
...
.teardown={}});
variants.push_back({.name="duplicate_content_length"sv,
.setup=error_setup,
.run=error_run_reconnect(kDuplicateCL),
.teardown=plain_teardown});
variants.push_back({.name="conflicting_content_length"sv,
.setup=error_setup,
.run=error_run_reconnect(kConflictingCL),
.teardown=plain_teardown});
variants.push_back({.name="te_plus_content_length"sv,
.setup=error_setup,
.run=error_run_reconnect(kTEPlusCL),
.teardown=plain_teardown});
variants.push_back({.name="obs_fold_header"sv,
.setup=error_setup,
.run=error_run_reconnect(kObsFold),
.teardown=plain_teardown});
variants.push_back({.name="invalid_header_whitespace"sv,
.setup=error_setup,
.run=error_run_reconnect(kInvalidHeaderWS),
.teardown=plain_teardown});
variants.push_back({.name="invalid_te_value"sv,
.setup=error_setup,
...
variants.push_back({.name="pipeline_good_bad_good"sv,
.setup=error_setup,
.run=[&]{
...
.teardown=plain_teardown});
variants.push_back({.name="pipeline_bad_after_good"sv,
.setup=error_setup,
.run=[&]{
...
.teardown=plain_teardown});
variants.push_back({.name="pipeline_oversized_header"sv,
.setup=error_setup,
.run=[&]{
...
variants.push_back({.name="client_shutdown_wr_after_req"sv,
.setup=error_setup,
.run=[&]{
client->send_all(kGetJson);
client->shutdown_wr();
...
.teardown=plain_teardown});
variants.push_back({.name="client_shutdown_wr_mid_body"sv,
.setup={},
.run=[&]{
BenchClient c{plain.port};
...
variants.push_back({.name="slow_reader_64k"sv,
.setup=plain_setup,
.run=[&]{
...
.teardown=plain_teardown,
.iters_override=50});
variants.push_back({.name="slow_reader_1m"sv,
.setup=plain_setup,
.run=[&]{
...
.teardown=plain_teardown,
.iters_override=10});
variants.push_back({.name="client_stalls_after_headers"sv,
.setup=plain_setup,
.run=[&]{
client->send_all(kGetBody64k);
...
.teardown=plain_teardown,
.iters_override=50});
variants.push_back({.name="client_close_during_send"sv,
.setup={},
.run=[&]{
BenchClient c{plain.port};
...
variants.push_back({.name="sse_3_events"sv,
.setup=plain_setup,
.run=[&]{
...
variants.push_back({.name="sse_client_disconnect"sv,
.setup={},
.run=[&]{
BenchClient c{plain.port};
c.send_all(kGetSSE);
...
variants.push_back({.name="ws_upgrade"sv,
.setup={},
.run=[&]{
...
.teardown={}});
variants.push_back({.name="ws_echo_100"sv,
.setup={},
.run=[&]{
auto[c,resp]=ws_upgrade_req(plain.port);
A<char,4096>rbuf{};
...
.teardown={},
.ops_per_iter=100});
variants.push_back({.name="ws_client_disconnect"sv,
.setup={},
.run=[&]{
auto[c,resp]=ws_upgrade_req(plain.port);
c.send_all(ws_text_frame);
...
auto const kGetStaticEtagHit=format(
"GET /1k.txt HTTP/1.1\r\nHost: localhost\r\nIf-None-Match: {}\r\n\r\n",static_etag);

variants.push_back({.name="static_1k"sv,
.setup=static_setup,
.run=[&]{client->send_all(kGetStatic1k);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="static_64k"sv,
.setup=static_setup,
.run=[&]{client->send_all(kGetStatic64k);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="static_1m"sv,
.setup=static_setup,
.run=[&]{client->send_all(kGetStatic1m);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="static_etag_hit"sv,
.setup=static_setup,
.run=[&]{client->send_all(kGetStaticEtagHit);(void)client->recv_response_no_body(sb);},
.teardown=plain_teardown});
variants.push_back({.name="static_range"sv,
.setup=static_setup,
.run=[&]{client->send_all(kGetStaticRange);(void)client->recv_response(sb);},
.teardown=plain_teardown});
...
variants.push_back({.name="deferred_ok"sv,
.setup=[&]{client=make_unique<BenchClient>(deferred.port);},
.run=[&]{client->send_all(kGetDefer);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="deferred_timeout_cancel"sv,
.setup={},
.run=[&]{
...
.teardown={}});
variants.push_back({.name="deferred_many_inflight"sv,
.setup={},
.run=[&]{
...
variants.push_back({.name="rN_get_json"sv,
.setup=rn_setup,
.run=[&]{client->send_all(kGetJsonRN);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="rN_post_echo_4k"sv,
.setup=rn_setup,
.run=[&]{client->send_all(post_4k_rn);(void)client->recv_response(sb);},
.teardown=plain_teardown});
variants.push_back({.name="rN_connect_close"sv,
.setup={},
.run=[&]{BenchClient c{plain_rn.port};c.send_all(kGetJsonRN);(void)c.recv_response(sb);},
.teardown={}});
variants.push_back({.name="rN_get_body_64k"sv,
.setup=rn_setup,
.run=[&]{client->send_all(kGetBody64kRN);(void)client->recv_response(lb);},
.teardown=plain_teardown});
...
variants.push_back({.name="sr32_pipeline_100"sv,
.setup=[&]{client=make_unique<BenchClient>(small_ring_32.port);},
.run=[&]{
for(int i=0;i<100;++i)client->send_all(kGetJson);
(void)client->recv_n_responses(lb,100);},
.teardown=plain_teardown,
.ops_per_iter=100});
variants.push_back({.name="sr32_get_body_1m"sv,
.setup=[&]{client=make_unique<BenchClient>(small_ring_32.port);},
.run=[&]{client->send_all(kGetBody1m);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="sr64_connect_close"sv,
.setup={},
.run=[&]{
...
variants.push_back({.name="br64_post_echo_64k"sv,
.setup=[&]{client=make_unique<BenchClient>(small_buf_ring_64.port);},
.run=[&]{client->send_all(post_64k_br);(void)client->recv_response(lb);},
.teardown=plain_teardown});
variants.push_back({.name="br64_pipeline_100"sv,
.setup=[&]{client=make_unique<BenchClient>(small_buf_ring_64.port);},
.run=[&]{
for(int i=0;i<100;++i)client->send_all(kGetJson);
(void)client->recv_n_responses(lb,100);},
.teardown=plain_teardown,
.ops_per_iter=100});
variants.push_back({.name="br128_get_body_64k"sv,
